...
storiesOf('Components|Content/JobCard', module)
  ...
  .add(
    'Default',
    () => {
      const type : JobCardType = select('Darstellung', {'Job-Card': 'CARD', 'Job-List': 'LIST'}, 'LIST');
      const loading = boolean('Ladestatus', false);
      const p = [type=== 'CARD' ? 1: 0, type=== 'CARD' ? 1 : 0 ,0];

      const job = {
        companyName: text('Firmennanme','PB3C GmbH'),
        imageUrl: text('Firmenlogo','https://storage.googleapis.com/t3n-de/jobslogos/50ee624bec9641caa70b3ff15300b75157ef532b/pb3c.png'),
        title: text('Jobtitel','(Senior) Performance Marketing Manager (m/w/d)'),
        url: text('Job-URL','https://t3n.de/jobs/job/pb3c-gmbh/senior-performance-marketing-manager-m-w-d/'),
